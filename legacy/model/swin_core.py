...
class WindowAttention(nn.Module):
    ...
    def __init__(
        ...
        if context_dim is None:
            ...
            self.kv = nn.Linear(context_dim, attn_dim * 2, bias=qkv_bias)
    ...
    def forward(self, x, mask: Optional[torch.Tensor] = None, context=None):
        """
        Args:
        ...
        """
        B_, N, _ = x.shape
        assert self.context_dim is None or context is not None
        if context is None:
            qkv = self.qkv(x).reshape(B_, N, 3, self.num_heads, -1).permute(2, 0, 3, 1, 4)
            q, k, v = qkv.unbind(0)  # make torchscript happy (cannot use tensor as tuple)
        else:
            kv = self.kv(context).reshape(B_, N, 2, self.num_heads, -1).permute(2, 0, 3, 1, 4)
            k, v = kv.unbind(0)  # make torchscript happy (cannot use tensor as tuple)
            q = self.q(x).reshape(B_, N, 1, self.num_heads, -1).permute(2, 0, 3, 1, 4)
            (q,) = q.unbind(0)
        ...
        if mask is not None:
            ...
            attn = attn.view(B_ // num_win, num_win, self.num_heads, N, N) + mask.unsqueeze(
                1
            ).unsqueeze(0)
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, context=None):
        ...
        x = self.norm1(x)
        x = x.view(B, H, W, C)
        context = None if context is None else context.view(B, H, W, -1)
        ...
        if self.shift_size > 0:
            ...
            shifted_context = (
                None
                if context is None
                else torch.roll(context, shifts=(-self.shift_size, -self.shift_size), dims=(1, 2))
            )
        else:
            shifted_x = x
        ...
        if shifted_context is not None:
            context_windows = window_partition(shifted_context, self.window_size)
            context_windows = context_windows.view(
                -1, self.window_size * self.window_size, context_windows.shape[-1]
            )

        # W-MSA/SW-MSA
        attn_windows = self.attn(
            x_windows, mask=self.attn_mask, context=context_windows
        )  # num_win*B, window_size*window_size, C
        ...
        self.blocks = nn.Sequential(
            *[
                SwinTransformerBlock(
                    ...
                    context_dim=context_dim if i % 4 in [2, 3] else None,
                )
    ...
    def forward(self, x, context=None):
        if context is None:
            ...
            for i, blk in enumerate(self.blocks):
                con = context if i % 4 in [2, 3] else None
                x = blk(x, context=con)
        ...
        for i in range(self.num_layers):
            layers += [
                BasicLayer(
                    ...
                    norm_layer=norm_layer,
                    downsample=block_end_module if (i < self.num_layers - 1) else None,
                    context_dim=embed_dim[i] if use_context else None,
                )
            ]
...
# ------------------------------------------------------------------------------
# END OF ORIGINAL SWIN FILE FROM TIMM, NOW MY IMPLEMENTATION
# ------------------------------------------------------------------------------
...
class SwinUNetEncoder(nn.Module):
    def __init__(
        ...
        super().__init__()
        num_heads = [int(first_dim // head_dim) * (i + 1) for i in range(len(depths))]
        self.tsfm = SwinTransformer(
            img_size=img_size,
            ...
            depths=depths,
            num_heads=num_heads,
            block_end_module=None,
            mlp_ratio=mlp_ratio,
            # drop_path_rate=0,
        )
        embed_dim = [int(first_dim * 2**i) for i in range(self.tsfm.num_layers)]
        input_resolution = [
            (
                self.tsfm.patch_grid[0] // (2**i),
                self.tsfm.patch_grid[1] // (2**i),
            )
            for i in range(self.tsfm.num_layers)
        ]
        self.downsample = nn.ModuleList(
            [
                PatchMerging(input_resolution[i], embed_dim[i], embed_dim[i + 1])
                for i in range(self.tsfm.num_layers - 1)
            ]
            + [nn.Identity()]
        )

    def forward(self, x, view_emb=None):
        x = self.tsfm.patch_embed(x)
        if self.tsfm.absolute_pos_embed is not None:
            x = x + self.tsfm.absolute_pos_embed
        x = self.tsfm.pos_drop(x)
        skips = {}
        for i in range(self.tsfm.num_layers):
            if view_emb is not None:
                x = x + view_emb[i]
            x = self.tsfm.layers[i](x)
            skips[i] = x
...
class PatchExpanding(nn.Module):
    ...
    def __init__(self, input_resolution, dim, out_dim=None, norm_layer=nn.LayerNorm):
        ...
        self.expansion = nn.Linear(self.dim, 4 * self.out_dim, bias=False)
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 3, 1, 2)  # B(2C)HW
        x = nn.functional.pixel_shuffle(x, 2)  # B(C/2)(2H)(2W)
...
class SwinUNetDecoder(nn.Module):
    def __init__(
        ...
        self.tsfm = SwinTransformer(
            ...
            depths=depths,
            num_heads=num_heads,
            block_end_module=None,  # don't downsample
            mlp_ratio=mlp_ratio,
            # drop_path_rate=0,
            use_context=use_context,
        )

        # collapse skip connection to same dimension
        embed_dim = [int(first_dim * 2**i) for i in range(self.tsfm.num_layers)]
        self.skip_emb = nn.ModuleList([nn.Linear((1 + num_skips) * e, e) for e in embed_dim])
        ...
        # do upsample instead of the downsample
        self.upsample = nn.ModuleList(
            [nn.Identity()]
            + [
                PatchExpanding(input_resolution[i], embed_dim[i], embed_dim[i - 1])
                for i in range(1, self.tsfm.num_layers)
            ]
        )

    def forward(self, x, skips, context=None, view_emb=None):
        skips_dec = {}
        for i in list(range(self.tsfm.num_layers))[::-1]:
            x = torch.cat([x, skips[i]], -1)
            x = self.skip_emb[i](x)
            if view_emb is not None:
                x = x + view_emb[i]
            con = None if context is None else context[i]
            x = self.tsfm.layers[i](x, context=con)
            skips_dec[i] = x
            x = self.upsample[i](x)
        return x, skips_dec
